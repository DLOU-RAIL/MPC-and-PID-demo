...
class PoseToPose_MPC(object): # PoseToPose Mpc 类
	...
	# 车辆模型
    def carModel(self, prev_state, dt, pedal, steering): # 上一个时态的状态，时间间隔，踏板，转向
        x = prev_state[0] # x = 状态列表 [0]
        y = prev_state[1] # y = 状态列表 [1]
        yaw = prev_state[2] # yaw = 状态列表 [2]
        v = prev_state[3] # v=状态列表 [3]
	...
	# 预测步
        for k in range(0, self.horizon):
            v_start = state[3] # 当前的速度
            yaw_start = state[2] #当前的偏航角
	...
	# 状态 = [里程计的x位置的姿态，里程计的y位置的姿态，偏航角，里程计的x位置的姿态，里程计的转动线性x]
        ...
        u = np.append(u, u[-2])
        u = np.append(u, u[-2])
	# u初始迭代值 （state，ref）目标函数参数，算法的名字 ，变量的界限 ， 迭代停止的精度 10*（-5）
	...
	# predicted = 上一个车辆时态的 
        ...
        return u, predicted_state # 返回 生成包含[预测步 * num_inputs]个元素的零矩阵，预测的状态

    
    def calculateTwistCommandHorizonPath(self): # 计算控制车辆预测步路径矩阵
        u, predicted_state = self.Optimizer() # 目标值，预测状态
        # calculate twist command # 计算控制车辆矩阵
        pedal = u[0] #  加速度
        steering = u[1]	# 转向
        twistCmd = Twist()
	# v = linear.x + pedal * 1/30 (Rate 比率）
        vel_cmd = self.odom.twist.twist.linear.x + pedal * 1.0/self.Rate  # 速度 = 线速度+ 加速度*1/30
        if vel_cmd < -self.MaxSpeed: # 速度<负的最大速度
            twistCmd.linear.x = -self.MaxSpeed # 线速度 = 负的最大速度 
        elif vel_cmd > self.MaxSpeed: # 速度>负的最大速度
            twistCmd.linear.x = self.MaxSpeed # 线速度 = 最大速度 
        else:	# 如果速度 = 最大速度	
            twistCmd.linear.x = vel_cmd # 线速度 = 速度
        twistCmd.angular.z = steering # 速度转角 = 转向
